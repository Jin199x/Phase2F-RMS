                ...
                using (SqlConnection conn = DatabaseHelper.GetConnection())
                {
                    ...
                    using (SqlTransaction tran = conn.BeginTransaction())
                    {
                        try
                        {
                            ...
                            using (SqlCommand cmd = new SqlCommand(@"UPDATE HomeownerUnits SET IsCurrent = 0, 
                                DateOfOwnershipEnd = ISNULL(DateOfOwnershipEnd, GETDATE()) WHERE ResidentID = @residentId AND UnitID = @unitId AND IsCurrent = 1", conn, tran))
                            {
                                cmd.Parameters.AddWithValue("@residentId", residentId);
                                cmd.Parameters.AddWithValue("@unitId", unitId);
                                cmd.ExecuteNonQuery();
                            ...
                            using (SqlCommand cmd = new SqlCommand(@"UPDATE HomeownerUnits SET IsCurrent = 0, DateOfOwnershipEnd = ISNULL(DateOfOwnershipEnd,
                                GETDATE()) WHERE UnitID = @unitId AND ResidentID IN (SELECT ResidentID FROM Residents WHERE ResidencyType IN ('Tenant', 'Caretaker')) AND IsCurrent = 1", conn, tran))
                            {
                                cmd.Parameters.AddWithValue("@unitId", unitId);
                                cmd.ExecuteNonQuery();
                            }
        ...
        private void ShowResidentUnits(int residentId)
        {
            try
            {
                using (SqlConnection conn = DatabaseHelper.GetConnection())
                {
                    ...
                    searchPanel.Controls.Add(lblUnitNumber);

                    TextBox txtUnitNumber = new TextBox
                    {
                    ...
                    searchPanel.Controls.Add(txtUnitNumber);
                    ...
                    searchPanel.Controls.Add(lblBlock);
                    ...
                    searchPanel.Controls.Add(txtBlock);

                    int clearButtonX = txtBlock.Location.X + txtBlock.Width + 10;

                    Button btnClear = new Button
                    {
                        Text = "Clear",
                        Location = new Point(clearButtonX, 9),
                        Width = 70,
                        Height = 32,
                    ...
                    layout.Controls.Add(dgvPanel, 0, 1);


                    void ApplyFilter()
                    {
                        string unitSearch = txtUnitNumber.Text.Trim().ToLower();
                        string blockSearch = txtBlock.Text.Trim().ToLower();

                        var dtSource = dgv.DataSource as DataTable;
                        if (dtSource == null) return;

                        if (string.IsNullOrEmpty(unitSearch) && string.IsNullOrEmpty(blockSearch))
                        {
                        ...
                        else
                        {
                            ...
                            if (!string.IsNullOrEmpty(unitSearch))
                                filters.Add($"CONVERT(UnitNumber, 'System.String') LIKE '%{unitSearch}%'");

                            if (!string.IsNullOrEmpty(blockSearch))
                                filters.Add($"CONVERT(Block, 'System.String') LIKE '%{blockSearch}%'");

                            dtSource.DefaultView.RowFilter = string.Join(" AND ", filters);
                        }
                    }

                    txtUnitNumber.TextChanged += (s, e) => ApplyFilter();
                    txtBlock.TextChanged += (s, e) => ApplyFilter();
            ...
            try
            {
                ...
                void ApplySearchFilter()
                {
                    ...
                    else
                    {
                        ...
                        (dgvUnits.DataSource as DataTable).DefaultView.RowFilter = filterExpression;
                    }
                }
                txtUnitNumber.TextChanged += (s, e) => ApplySearchFilter();
                txtBlock.TextChanged += (s, e) => ApplySearchFilter();
                btnClear.Click += (s, e) =>
                {
                    txtUnitNumber.Clear();
                    txtBlock.Clear();
